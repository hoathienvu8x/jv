...
int search(
  struct json_stream_t *stream, const char *chars, struct output_t *out
) {
  const char *chp;
  const char *start;
  int code;
...
int traverse_collection(
  struct json_stream_t *stream, char open, int count, struct output_t *out
) {
  ...
  while (count > 0) {
    ...
    } else if (ch == open && env != STRING) {
      count++;
    } else if (ch == close && env != STRING) {
      count--;
    }
  }
...
const char *scan_key(struct json_stream_t *stream, const char *path) {
  ...
  for (i = 0; i < path_key.len; i++) {
    if (path_key.value[i] != stream->pos[0]) {
      ...
      #ifdef JVDEBUG
      fprintf(
        stdout, "  mismatch (path: %c, object: %c)\n",
